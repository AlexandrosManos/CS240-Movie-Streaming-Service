...
struct user sen;
struct user *Users;
struct new_movie *NMovies;
struct movie *array[6];
...
void init_structures(void)
{
	 ...
	 */
	sen.uid = -1;
	sen.suggestedHead = NULL;
	sen.next = NULL;
	sen.suggestedTail = NULL;
	sen.watchHistory = NULL;
	Users = &sen;
	NMovies = NULL;
	for (int i = 0; i < 6; i++)
	{
...
void destroy_structures(void)
{
	 ...
	 */
	// free(NMovies); NMovies is already free (function distribute_new_movies)
	...
	for (int i = 0; i < 6; i++)
	{
		...
		struct movie *next_mov;
		while (array[i] != NULL)
		{
			next_mov = array[i]->next;
			free(array[i]);
			array[i] = next_mov;
		}
	}
